...
async def implete(
    SSML_text: str, opt_fmt: str, debug: bool, method: int = 1
    ...
    if method == 1:
        ...
        try:
            ...
            recentErrTime = time()
            log.error(
                "An unexpected exception occurred. If this error kept going, please make an Issue on github with code 1"
            )
        ...
        try:
            ...
            log.error(
                "An unexpected exception occurred. If this error kept going, please make an Issue on github with code %d"
                % method
            )
            log.info("We will use the backup method.")
        ...
        raise ValueError(
            f"Method must between 1 to 2, but '{method}' got. If you haven't specified 'method', this is because of all the methods failed. If so, please make an Issue."
        )


class TtsHelper:
    def run(self, text: str, opt: str):
        SSML_text = f"""<speak xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="http://www.w3.org/2001/mstts" xmlns:emo="http://www.w3.org/2009/10/emotionml" version="1.0" xml:lang="en-US">
    ...
    def createTask(self, SSML_text: str) -> asyncio.Task:
        return asyncio.get_event_loop().create_task(
            implete(SSML_text, "audio-24khz-48kbitrate-mono-mp3", True)
        )

    def _callback(self, future: asyncio.Future):
        ret = future.result()
        _, b = ret
        with open(self.opt, "wb") as f:
            f.write(b)
        # playsound(self.opt)
...
if __name__ == "__main__":
    ...
    t = TtsHelper().run("SSML_text", "test.mp3")
