...
from PySide6.QtGui import QCloseEvent, QColor, QImageReader, QKeyEvent, QPixmap
from PySide6.QtWidgets import (QApplication, QErrorMessage, QGridLayout,
                               QLabel, QMessageBox, QSizePolicy, QTableWidget,
...
class AutoChoose(QObject):
    def __init__(self, Form: QWidget, worker: Worker) -> None:
        ...
        try:
            ...
            QErrorMessage().showMessage(
                f"Cannot decode {REMOVE_CFG}: <br>"
                + format_exc(e).replace("\n", "<br>")
            )
    ...
    def closeEv(self, event: QCloseEvent):
        ...
        if QMessageBox.question(self._form, "退出", "退出软件？", btn_yes, btn_no) == btn_yes:
            event.accept()
        else:
            event.ignore()
    ...
    def play(self, filename: str):
        ...
        Popen(
            "ffplay -loglevel error -hide_banner -nodisp -autoexit "
            + str(file.absolute()),
            stdout=None,
        )
    ...
    def setupImgs(self):
        ...
        if not IMG_DIR.is_dir():
            self.worker.run(
                "Target dir <b>'{}'</b> not exists.".format(str(IMG_DIR)), True
            )
            return
        ...
        for name in remove:
            warntxt.append(
                f"File <b>'{name}'</b> found in <b>'{REMOVE_CFG}'</b> but not in <b>'{IMG_DIR}'</b>, autoremoved."
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        err = QErrorMessage()
        err.showMessage(
            "Failed to inititalize 'AutoChoose'<br>"
            + format_exc().replace("\n", "<br>")
        )
        err.exec()
    ui.show()
    try:
        ...
        QErrorMessage().showMessage(
            "Failed to inititalize Floating Window<br>"
            + format_exc().replace("\n", "<br>")
        )
